...
**Layout**
22. `_side_room` reserved space against a cursor its caller was about to overwrite — the check ran *before* the block's own leading gap was applied — and reserved past the block's last baseline into the whitespace that follows it. Together they over-reserved **15 to 60 points on every call**, so any sidebar ending within ~15pt of the foot of the page was broken spuriously. One real CV's sidebar ended at 760.6 against a limit of 770 and was still pushed onto a second page. Replaced by `_side_place(start, height)`, where `start` is where the block actually lands and `height` runs to its last baseline.
23. Sidebar section headings had no keep-with-next: `side_heading` reserved room for its own glyphs and nothing for the first line of what it introduces, so `FORMATION` sat alone at the foot of a page with the degrees overleaf. The main column had always done this for a role's title ("keep a role's title with at least its first line of evidence"); the sidebar never got the equivalent.
24. Nothing ever looked at the finished document. `finish()` counts pages as `max(page index claimed by any op) + 1`, so **one text run on page two costs a whole physical sheet** — and `_furniture()` paints the full-height teal band on it regardless. A visitor's CV shipped a second page that was 16.9% full, holding three lines. See `_fit_modern` in `builder.py`.
25. The suite rated all of the above as passing: `test_builder.py` asserted `pages <= 2` for a CV that fits on one, and `test_fidelity.py` pins fonts, palette and two geometry constants — **not one vertical rhythm value**. `tests/test_layout.py` and `tests/reference_draft.py` close that hole; the first test there asserts the reference CV renders at exactly the designed spacing.
...
* **`_cvmodern.py` / `_cvdesign.py` are vendored verbatim.** Geometry was measured off reference PDFs at 110dpi. Only three deliberate edits exist (font registration, employer weight, the scalable rhythm below), all tested.
* **The modern layout is fitted to the page, within a bounded range.** `_fit_modern` renders at the measured spacing, looks at the result, and re-renders only if the page is badly used: it tightens the column that spilled (down to `FIT_FLOOR` 0.72) to delete a page nobody asked for, and spreads a main column that ends above `SPARSE_MAIN` (0.82 of the page) out toward `COMFORTABLE` (0.93), never past `FIT_CEILING` 1.18. Four things keep this honest and none of them are optional: it scales **only the space between blocks** — never type size, never leading, never the content; the two columns get **separate** scales, because tightening a sidebar to save a page must not also pull up a main column that is already ending short; a CV that will not fit inside the range is shipped **long, at the measured rhythm**, rather than squeezed; and a page that is already well used is **not re-rendered at all**, which is what keeps the reference CV (92% of the main column) bit-identical. It is cheap because `ModernCV` draws nothing until `finish()` — a trial render is arithmetic over the same parsed draft. Worst case is six renders, ~77ms total.
